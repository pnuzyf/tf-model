...
class MMoEModel(tf.keras.Model, ABC):
    def __init__(self, model_input_config: ModelInputConfig, task_structs, num_experts, expert_layers,
                 ...
                 gate_l1_reg=None, gate_l2_reg=None, share_gates=False, named_outputs=None,
                 use_wide=False, common_wide=True, cin_layer=None, dim=8, name='MMoE'):
        ...
        for struct, use_bias, hidden_act, output_act, use_bn, dropout, l1_reg, l2_reg \
            ...
            if use_wide and not common_wide:
                self.cin_layer.append(CINLayer(cin_layer, activate='relu', use_bias=True))
                for input_config in model_input_config.get_inputs_by_group("wide_{}".format(idx)):
                    wide_feat.append(input_config.name)

                self.wide_feats[idx] = wide_feat
            idx += 1

        #共享结构
        if use_wide and common_wide:
            self.cin_layer.append(CINLayer(cin_layer, dim, activate='relu', use_bias=True))
            wide_feat = []
            for input_config in model_input_config.get_inputs_by_group("wide"):
                wide_feat.append(input_config.name)
            self.wide_feats[0] = wide_feat
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        feat_vals = []
        wide_input = defaultdict(list)

        for input_config in self.model_input_config.all_inputs:
            ...
            if self.use_wide:
                for k, v in self.wide_feats.items():
                    if name in v:
                        wide_input[k].append(m)


        wide_logits = [] #多个输出
        if self.use_wide:
            for index, value in wide_input.items():
                stacked_wide_vals = tf.stack(value, axis=1)
                wide_logit, wide_cross_feats = self.cin_layer[index](stacked_wide_vals)
                wide_logits.append(wide_logit)
        ...
        if self.named_outputs:
            ...
            for name, output, task_act_layer in zip(self.label_names, task_outputs, self.task_active_layers):
                if wide_logits is not None and self.use_wide:
                    if self.common_wide:
                        output = output + wide_logits[0]
                    else:
                        output = output + wide_logits[idx]
                if task_act_layer:
