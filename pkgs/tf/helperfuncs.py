...
def create_loss(loss_type, **kwargs):
    ...
    if loss_type in ['truncated_mse', 'tmse']:
        from .extend_losses import TruncatedMseLoss
        return TruncatedMseLoss(**kwargs)
    if loss_type in ['infonce']:
        from .extend_losses import InfoNceLoss
...
def create_metric(metric_type, name_prefix, **kwargs):
    ...
    if metric_type in ['softmax_acc']:
        from .extend_metrics import SoftmaxAccuracy
        return SoftmaxAccuracy(**kwargs)
