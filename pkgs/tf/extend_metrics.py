...
def softmax_acc(y_true, y_pred, **kwargs):
    # [batch, batch]

    num_queries = tf.shape(y_pred)[0]
    num_candidates = tf.shape(y_pred)[1]
    y_pred = tf.eye(num_queries, num_candidates)
    y_true_label = tf.argmax(y_true)
    y_pred_label = tf.argmax(y_pred)
    # tf.print([y_true_label, y_pred_label])
    output = tf.cast(tf.not_equal(y_true_label, y_pred_label), tf.float32)

    return output

class SoftmaxAccuracy(MeanMetricWrapper):
    def __init__(self, name="softmax_acc", dtype=None):
        super(SoftmaxAccuracy, self).__init__(softmax_acc, name=name, dtype=dtype)

    def get_config(self):
        config = super(SoftmaxAccuracy, self).get_config()
        return config
