...
class PersonalLiveExpertV1(tf.keras.layers.Layer):
    def __init__(self, layer_id, task_name, expert_id, name='prev1_layer', trainable=True, **kwargs):
        super(PersonalLiveExpertV1, self).__init__(name=name, trainable=trainable, **kwargs)
        self.layer_id = layer_id # ple层的id
        self.task_name = task_name # 多任务的任务名称
        self.expert_id = expert_id # 专家id
        self.d1 = tf.keras.layers.Dense(64, kernel_regularizer=tf.keras.regularizers.l2(0.01), activation='relu', name='L%d_T%s_E%d_dense_relu_1' % (self.layer_id, self.task_name, self.expert_id))
    ...
    def get_config(self):
        config = super(PersonalLiveExpertV1, self).get_config()
        config.update({
            'layer_id': self.layer_id,
            'task_name': self.task_name,
            'expert_id': self.expert_id,
...
class PersonalLiveTowerV1(tf.keras.layers.Layer):
    def __init__(self, task_name, name='prtv1', trainable=True, **kwargs):
        super(PersonalLiveTowerV1, self).__init__(name=name, trainable=trainable, **kwargs)
        self.task_name = task_name
        self.seq1 = tf.keras.layers.Dense(32, kernel_regularizer=tf.keras.regularizers.l2(0.01), activation='relu',
                                          name='T%s_tower_dense' % (task_name))
        self.seq2 = tf.keras.layers.Dense(1, kernel_regularizer=tf.keras.regularizers.l2(0.01), activation=None,
                                          name='T%s_tower_dense' % (task_name))
    @tf.function
    def call(self, inputs, training=None, **kwargs):
        y = inputs
    ...
    def get_config(self):
        config = super(PersonalLiveTowerV1, self).get_config()
        config.update({
            'task_name': self.task_name,
            'name': self.name
        })
