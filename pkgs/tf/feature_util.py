...
class InputDesc(namedtuple('InputDesc',
                           ['name', 'dtype_str', 'shape', 'is_sparse', 'is_ragged', 'default_value', 'val_sep',
                            ...
                            'num_oov_buckets', 'self_weighted', 'query', 'other_keys', 'is_gate', 'is_filter', 'groups',
                            'is_long', 'is_short'])):
    ...
    def __new__(cls, name, dtype_str, shape=(1,), is_sparse=False, is_ragged=False, default_value=None,
                ...
                num_oov_buckets=None, self_weighted=False, query=None, other_keys=None, is_gate=False, is_filter=False,
                groups=None, is_long=False, is_short=False):
        return super(InputDesc, cls).__new__(cls, name, dtype_str, shape, is_sparse, is_ragged, default_value,
                                             ...
                                             self_weighted, query, other_keys, is_gate, is_filter, groups,
                                             is_long, is_short)
    ...
    def parse_from_json(cls, conf_json):
        ...
        query = conf_json.get("query", None)
        other_keys = conf_json.get("other_keys", None)
        #添加单特征来进行特征权重控制
        is_gate = conf_json.get("is_gate", False)
        is_filter = conf_json.get("is_filter", False)
        exclude = conf_json.get('exclude', False)
        max_len = conf_json.get('max_len', None)
        padding = conf_json.get('padding', None)
        num_oov_buckets = conf_json.get('num_oov_buckets', None)
        groups = conf_json.get('groups')
        is_long = conf_json.get("is_long", False)
        is_short = conf_json.get("is_short", False)
        ...
        return InputDesc(name, conf_json.get('dtype'), shape, is_sparse, is_ragged, default_value, val_sep, is_label,
                         ...
                         exclude, max_len, padding, num_oov_buckets, self_weighted, query, other_keys, is_gate, is_filter, groups,
                         is_long, is_short)
